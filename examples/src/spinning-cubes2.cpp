...
void initVAO() {
	...
	// only draw cubes if they have nothing in front of them
...
int main(void) {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		for (int i = 0; i < 24; i++) {
			...
			mv_matrix = glm::rotate(mv_matrix, glm::radians(time * 21.0f), glm::vec3(1.0f, 0.0f, 0.0f));
			mv_matrix = glm::translate(mv_matrix,
									   glm::vec3(std::sin(2.1f * f) * 2.0f, std::cos(1.7f * f) * 2.0f, std::sin(1.3f * f) * std::cos(1.5f * f) * 2.0f));
