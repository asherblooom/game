...
int main(void) {
	...
	// create window
	GLFWwindow* window =
		glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Triangle Example", NULL, NULL);
	if (window == NULL) {
	...
	float* data = new float[256 * 256 * sizeof(float)];
	generate_texture(data, 256, 256);
	// fill texture with texture data
	glTexSubImage2D(GL_TEXTURE_2D,
					...
					GL_FLOAT,
					data);
	// don't need this anymore as opengl now has our texture data
	delete[] data;
